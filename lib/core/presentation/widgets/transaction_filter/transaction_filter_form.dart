...
class TransactionFilterForm extends StatefulWidget {
  ...
  /// Whether to show the transaction-specific refinements (tags, category,
  /// amount, type and status). Only [account] (and optionally the date) act as
  /// a universal scope; the rest are meaningless for stats not derived from
  /// transactions (e.g. net worth / portfolio), so they can be hidden there.
...
class _TransactionFilterFormState extends State<TransactionFilterForm> {
  ...
  Widget build(BuildContext context) {
    ...
    final hiddenRefinementCount = widget.showTransactionRefinements
        ? 0
        : widget.filter.transactionRefinementFilterCount;

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        /* ---------------------------------- */
        /* --- INAPPLICABLE FILTERS INFO ---- */
        /* ---------------------------------- */
        if (hiddenRefinementCount > 0) ...[
          InlineInfoCard(
            mode: InlineInfoCardMode.info,
            text: t.transaction.filters.inapplicable_here(
              n: hiddenRefinementCount,
            ),
          ),
        ...
        /* ---------- TAGS SELECTOR -------- */
        /* ---------------------------------- */
        if (widget.showTransactionRefinements) ...[
          ...
          StreamBuilder(
            ...
            builder: (context, snapshot) {
              List<Category>? selectedCategories =
                  ...
                  : (snapshot.data ?? [])
                        .where(
                          (element) => filtersToReturn.categoriesIds!.contains(
                            element.id,
                          ),
                        )
                        .toList();
        ...
        /* ---------------------------------- */
        if (widget.showTransactionRefinements) ...[
          SizedBox(height: widget.showDateFilter ? 16 : 24),
          StreamBuilder(
